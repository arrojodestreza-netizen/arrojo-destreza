...
export default function App() {
  ...
  const analyze = async () => {
    ...
    try {
      ...
      /* CORREÇÃO — a instrução de idioma passa a vir no FIM do prompt (depois das
         secções), com um aviso final reforçado. A IA tende a dar mais peso às
         instruções mais próximas do ponto onde começa a gerar a resposta — colocar
         o idioma só a meio do prompt (como estava antes) permitia que a "puxada" do
         português dos documentos anexados prevalecesse em partes da análise. */
...
${t("aiPrompt.language")}
FINAL REMINDER — OUTPUT LANGUAGE = ${langName.toUpperCase()}. The source financial documents are written in Portuguese, but that is only the source data — it does NOT set your output language. Write 100% of your response in ${langName}: every heading, label, sentence and number description. Do not switch languages mid-response, even briefly.` });

      setProgress(60);
      const res1 = await fetch(WORKER_URL, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ type: "analyze", payload: { model: "claude-sonnet-4-5", max_tokens: 4000, system: `You are an expert financial analyst who writes exclusively in ${langName}. CRITICAL INSTRUCTION: your ENTIRE response — every single word — must be in ${langName}. The attached financial documents are written in Portuguese, but that is only source data for you to analyze; you must never let their language leak into your own writing. If you notice yourself starting to write in Portuguese (or any language other than ${langName}), stop and continue in ${langName} instead. Do not mix languages within a sentence, a paragraph, or the response as a whole.`, messages: [{ role: "user", content }] } }) });
