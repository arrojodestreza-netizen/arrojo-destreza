...
${t("aiPrompt.sections")}` });
      ...
      // Chamada separada para posicionamento sectorial (só se CAE preenchido)
      if (cae && parsed) {
        try {
          const res3 = await fetch(WORKER_URL, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ type: "analyze", payload: { model: "claude-sonnet-4-5", max_tokens: 2000, system: "Respond ONLY with valid JSON. No markdown, no code blocks.", messages: [{ role: "user", content: `Based on this financial analysis, generate the sector positioning section as JSON.\n\nANALYSIS:\n${analiseTexto}\n\nCAE: ${cae}\nSECTOR: ${getCaeSector(cae)}\n\nRespond ONLY with this JSON structure:\n{"cae":"${cae}","setor":"${getCaeSector(cae)}","texto":"2-3 sentence analysis comparing company to sector average","indicadores":[{"nome":"Liquidez Geral","empresa":0.0,"setor":0.0,"posicao":"acima","positivo":true},{"nome":"Autonomia Financeira","empresa":0.0,"setor":0.0,"posicao":"abaixo","positivo":true},{"nome":"ROE","empresa":0.0,"setor":0.0,"posicao":"acima","positivo":true},{"nome":"ROA","empresa":0.0,"setor":0.0,"posicao":"acima","positivo":true},{"nome":"Margem Líquida","empresa":0.0,"setor":0.0,"posicao":"acima","positivo":true},{"nome":"Margem EBITDA","empresa":0.0,"setor":0.0,"posicao":"acima","positivo":true},{"nome":"Dívida/EBITDA","empresa":0.0,"setor":0.0,"posicao":"abaixo","positivo":false},{"nome":"PMR","empresa":0,"setor":0,"posicao":"acima","positivo":false},{"nome":"PMP","empresa":0,"setor":0,"posicao":"ok","positivo":true}]}` }] } }) });
          if (res3.ok) {
            const data3 = await res3.json();
            const text3 = data3.content?.map(b => b.text || "").join("\n") || "";
            const cleanText3 = text3.replace(/```json|```/g, "").trim().slice(text3.indexOf("{"), text3.lastIndexOf("}") + 1);
            try {
              const posicionamento = JSON.parse(cleanText3);
              parsed.posicionamento_sectorial = posicionamento;
            } catch(e) { /* ignorar erro no posicionamento — não bloquear o relatório */ }
          }
        } catch(e) { /* ignorar erro no posicionamento */ }
      }
