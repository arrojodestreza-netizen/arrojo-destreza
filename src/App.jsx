...
${t("aiPrompt.sections")}` });
      ...
      const res2 = await fetch(WORKER_URL, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ type: "analyze", payload: { model: "claude-sonnet-4-5", max_tokens: 6000, system: "Convert financial analysis text to valid JSON. Respond ONLY with JSON, no markdown, no code blocks, no additional text. Response starts with { and ends with }.", messages: [{ role: "user", content: `Convert this financial analysis to JSON with exactly this structure. Use real values from the text. Respond only with valid JSON:\n\nANALYSIS:\n${analiseTexto}\n\nJSON STRUCTURE:\n{"empresa":"${company}","cae":"${cae}","setor":"${cae ? getCaeSector(cae) : ""}","anos":${JSON.stringify(yearsFiltered)},"avaliacao_global":"Bom","sumario":{"texto":"summary text","destaques":["highlight 1","highlight 2","highlight 3"]},"financeiros":{"volume_negocios":${JSON.stringify(Object.fromEntries(yearsFiltered.map(y => [y, 0])))},"resultado_liquido":${JSON.stringify(Object.fromEntries(yearsFiltered.map(y => [y, 0])))},"ebitda":${JSON.stringify(Object.fromEntries(yearsFiltered.map(y => [y, 0])))},"total_ativo":${JSON.stringify(Object.fromEntries(yearsFiltered.map(y => [y, 0])))},"capital_proprio":${JSON.stringify(Object.fromEntries(yearsFiltered.map(y => [y, 0])))},"divida_financeira":${JSON.stringify(Object.fromEntries(yearsFiltered.map(y => [y, 0])))}},"indicadores":{"liquidez_geral":${JSON.stringify({...Object.fromEntries(yearsFiltered.map(y=>[y,0])),"referencia":"> 1.5","descricao":"Current ratio"})},"liquidez_reduzida":${JSON.stringify({...Object.fromEntries(yearsFiltered.map(y=>[y,0])),"referencia":"> 1.0","descricao":"Quick ratio"})},"autonomia_financeira":${JSON.stringify({...Object.fromEntries(yearsFiltered.map(y=>[y,0])),"referencia":"> 33%","descricao":"Equity ratio"})},"roe":${JSON.stringify({...Object.fromEntries(yearsFiltered.map(y=>[y,0])),"referencia":"> 10%","descricao":"Return on equity"})},"roa":${JSON.stringify({...Object.fromEntries(yearsFiltered.map(y=>[y,0])),"referencia":"> 5%","descricao":"Return on assets"})},"margem_liquida":${JSON.stringify({...Object.fromEntries(yearsFiltered.map(y=>[y,0])),"referencia":"> 5%","descricao":"Net margin"})},"margem_ebitda":${JSON.stringify({...Object.fromEntries(yearsFiltered.map(y=>[y,0])),"referencia":"> 10%","descricao":"EBITDA margin"})},"divida_ebitda":${JSON.stringify({...Object.fromEntries(yearsFiltered.map(y=>[y,0])),"referencia":"< 3.0x","descricao":"Financial leverage"})},"pme":${JSON.stringify({...Object.fromEntries(yearsFiltered.map(y=>[y,0])),"referencia":"< 45 days","descricao":"Days sales outstanding"})},"pmp":${JSON.stringify({...Object.fromEntries(yearsFiltered.map(y=>[y,0])),"referencia":"30-60 days","descricao":"Days payable outstanding"})}},"analise_detalhada":{"rentabilidade":"text","liquidez_solvabilidade":"text","estrutura_capital":"text","eficiencia_operacional":"text"},"posicionamento_sectorial":{"cae":"${cae}","setor":"${cae ? getCaeSector(cae) : ""}","texto":"sector positioning analysis text","indicadores":[{"nome":"Liquidez Geral","empresa":0,"setor":0,"posicao":"acima","positivo":true},{"nome":"Autonomia Financeira","empresa":0,"setor":0,"posicao":"abaixo","positivo":true},{"nome":"ROE","empresa":0,"setor":0,"posicao":"acima","positivo":true},{"nome":"ROA","empresa":0,"setor":0,"posicao":"abaixo","positivo":true},{"nome":"Margem Líquida","empresa":0,"setor":0,"posicao":"acima","positivo":true},{"nome":"Margem EBITDA","empresa":0,"setor":0,"posicao":"acima","positivo":true},{"nome":"Dívida/EBITDA","empresa":0,"setor":0,"posicao":"abaixo","positivo":false},{"nome":"PMR","empresa":0,"setor":0,"posicao":"acima","positivo":false},{"nome":"PMP","empresa":0,"setor":0,"posicao":"ok","positivo":true}]},"swot":{"forcas":["strength 1"],"fraquezas":["weakness 1"],"oportunidades":["opportunity 1"],"riscos":["risk 1"]},"recomendacoes":{"imediato":["action 1"],"medio":["action 1"],"longo":["action 1"]},"conclusao":"conclusion text"}` }] } }) });
